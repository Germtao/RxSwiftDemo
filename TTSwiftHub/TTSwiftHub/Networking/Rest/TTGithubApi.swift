...
extension TTGithubAPI: TargetType, TTProductAPIType {
    ...
    var path: String {
        ...
        case .notifications, .markAsReadNotifications: return "/notifications"
    ...
    var sampleData: Data {
        ...
        case .markAsReadNotifications,
             ...
             .followUser,
             .unfollowUser:
            dataUrl = R.file.emptyObjectJson()
        }
    ...
    var headers: [String : String]? {
        if let token = TTAuthManager.shared.token {
            switch token.type() {
    ...
    var parameters: [String: Any]? {
        ...
        case .issues(_, let state, let page):
            params["state"] = state
            params["page"] = page
        case .issue(_, let number):
            params["number"] = number
        case .issueComments(_, let number, let page):
            params["number"] = number
            params["page"] = page
        case .commits(_, let page):
            params["page"] = page
        case .commit(_, let sha):
            params["sha"] = sha
        case .branches(_, let page):
            params["page"] = page
        case .branch(_, let name):
            params["name"] = name
        case .releases(_, let page):
            params["page"] = page
        case .release(_, let releaseId):
