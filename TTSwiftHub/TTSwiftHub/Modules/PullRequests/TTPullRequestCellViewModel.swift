//
//  TTPullRequestCellViewModel.swift
//  TTSwiftHub
//
...
class TTPullRequestCellViewModel: TTDefaultTableViewCellViewModel {
    let pullRequest: TTPullRequest
    
    let userSelected = PublishSubject<TTUser>()
    
    init(pullRequest: TTPullRequest) {
...
extension TTPullRequest {
    ...
    func attributedDetail() -> NSAttributedString {
        ...
        labels?.forEach {
            if let name = $0.name, let color = UIColor(hexString: $0.color ?? "") {
